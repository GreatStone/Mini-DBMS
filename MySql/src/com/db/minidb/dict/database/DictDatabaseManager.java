...
public class DictDatabaseManager {
	...
	public static DictDatabaseInfo readDatabaseInfoFromStream(InputStream is) {
		synchronized (lockObj) {
			...
			for (int i = 0; i < tableCount; i++) {
				...
				InputStream tis = FileTool.getInputStream(FileTool
						.openFile(DictTableManager.getTableDictfileFullPath(
								database, tableId)));
				DictTableInfo table = DictTableManager
						.readTableInfoFromStream(tis);
	...
	public static void writeDatabaseInfoToStream(DictDatabaseInfo database,
			OutputStream os) {
		synchronized (lockObj) {
			if (database == null) {
				return;
			...
			for (DictTableInfo table : database.getTables()) {
				...
				OutputStream tos = FileTool.getOutputStream(FileTool
						.openFile(DictTableManager
								.getTableDictFileFullPath(table)));
				DictTableManager.writeTableInfoToStream(table, tos);
			}
		}
	}

	public static void writeDatabaseInfoToFile(DictDatabaseInfo database,
			File file) {
		synchronized (lockObj) {
			if (database == null) {
				return;
