...
public class DictDatabaseListManager {
	...
	public static String getDatabaseListDictFileFullPath() {
		synchronized (lockObj) {
			...
			String fullPath = FileTool.openFileFullPath(path,
					getDatabaseListDictFileFullName() + ".dat");
			return fullPath;
		}
	}
	...
	private static DictDatabaseListInfo readDatabaseListFromStream(
			InputStream is) {
		synchronized (lockObj) {
			...
			for (int i = 0; i < databaseCount; i++) {
				...
				InputStream dis = FileTool.getInputStream(FileTool
						.openFile(DictDatabaseManager
								.getDatabaseDictFileFullPath(databaseId)));
				databases.add(DictDatabaseManager
						.readDatabaseInfoFromStream(dis));
			}
	...
	private static void writeDatabaseListToStream(
			DictDatabaseListInfo databaseList, OutputStream os) {
		synchronized (lockObj) {
	...
	public static void writeDatabaseListToFile(
			DictDatabaseListInfo databaseList, File file) {
		synchronized (lockObj) {
			if (databaseList == null) {
				return;
			}
