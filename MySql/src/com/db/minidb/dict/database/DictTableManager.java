...
public class DictTableManager {
	...
	public static String getTableDictFileFullName(DictDatabaseInfo database, int tableId) {
		synchronized (lockObj) {
			int width = Integer.parseInt(PropertiesFileManager.getProperty("Dict_DatabaseIdWidth"));
			String fullName = DictDatabaseManager.getDatabaseDictFileFullName(database) 
					+ "_" 
	...
	public static String getTableDictFileFullName(DictTableInfo table) {
		synchronized (lockObj) {
			int width = Integer.parseInt(PropertiesFileManager.getProperty("Dict_DatabaseIdWidth"));
			String fullName = DictDatabaseManager.getDatabaseDictFileFullName(table.getDatabase()) 
								+ "_" 
							+ FormatTool.integerFormatWidth(table.getTableId(), width);
			return fullName;
		}
	}
	public static String getTableDictfileFullPath(DictDatabaseInfo database, int tableId) {
		synchronized (lockObj) {
	...
	public static String displayTableInfo(DictTableInfo table) {
		synchronized (lockObj) {
			...
			bf.append("table: " + table.getTableId() + " " + table.getTableName() + " have columns: " + table.getColumnCount() + "\r\n");
			for(DictColumnInfo col : table.getColumns()) {
				bf.append("\t\t" + String.format("%15s", col.getColumnName() + " " + col.getType().getTypeEnum()));
			}
			return bf.toString();
		}
	}
