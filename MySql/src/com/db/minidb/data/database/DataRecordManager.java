...
import com.db.minidb.data.value.ValueManager;
...
public class DataRecordManager {
	...
	private static List<ValueBase> readValues(List<DictColumnInfo> columns, InputStream is) {
		synchronized (lockObj) {
			ArrayList<ValueBase> values = new ArrayList<ValueBase>();
			for(DictColumnInfo col : columns) {
				ValueBase value = ValueManager.createByType(col.getType().getTypeEnum().toString());
				values.add(ValueManager.readValueFromStream(value, is));
			}
			return values;
		}
	}
	public static DataRecord readRecord(List<DictColumnInfo> columns, InputStream is) {
		synchronized (lockObj) {
