...
public class DictCenterManager {
	...
	public static boolean addDatabase(String databaseName) {
		synchronized (lockObj) {
			...
			databaseList.setNextDatabaseId(databaseList.getNextDatabaseId() + 1);
			DictDatabaseListManager.writeDatabaseListToFile(databaseList,
					FileTool.openFile(DictDatabaseListManager.getDatabaseListDictFileFullPath()));
			return false;
		}
	...
	public static boolean addTable(DictDatabaseInfo database, String tableName, String[] columnsName, String[] columnsType, Session session) {
		synchronized (lockObj) {
			addTable(database, tableName, columnsName, columnsType);
			DictDatabaseManager.writeDatabaseInfoToFile(database, 
					FileTool.openFile(DictDatabaseManager.getDatabaseDictFileFullPath(database)));
			return false;
		}
	}
	public static boolean addTable(DictDatabaseInfo database, String tableName, String[] columnsName, String[] columnsType) {
		synchronized (lockObj) {
			DictTableInfo table = createTable(database, tableName, columnsName, columnsType);
	...
	public static boolean removeTable(String databaseName, String tableName, Session session) {
		synchronized (lockObj) {
			return false;
		}
	}
	public static DictTableInfo findTableWithName(DictDatabaseInfo database, String tableName) {
		synchronized (lockObj) {
	...
	public static List<DictColumnInfo> createColumns(String[] columnsName, String[] columnsType) {
		synchronized (lockObj) {
			ArrayList<DictColumnInfo> columns = new ArrayList<DictColumnInfo>();
			for(int i = 0; i < columnsType.length; i++) {
				String typeEnum = PropertiesFileManager.getProperty(columnsType[i]);
				DictColumnInfo col = new DictColumnInfo(TypeDataEnum.valueOf(typeEnum));
	...
	public static List<DictColumnInfo> createColumns(DictTableInfo table, String[] columnsName, String[] columnsType) {
		synchronized (lockObj) {
			List<DictColumnInfo> columns = createColumns(columnsName, columnsType);
...
//	public static DictTableInfo createTable(String tableName, String[] columnsName, String[] columnsType) {
//		synchronized (lockObj) {
...
//		}
//	}
	public static DictTableInfo createTable(DictDatabaseInfo database, String tableName, String[] columnsName, String[] columnsType) {
		synchronized (lockObj) {
			DictTableInfo table = new DictTableInfo();
